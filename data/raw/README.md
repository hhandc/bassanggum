...
deterministic checksum of its `.shp`, `.dbf`, `.prj`, and `.cpg` components.
...
`filename\0component-sha256\n` entries for the `.shp`, `.dbf`, `.prj`, and
`.cpg` source files, so any component change changes provenance. `--kdpa-only
